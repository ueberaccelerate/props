...
namespace property
{
      ...
      if (holder_object.parent)
      {
        ...
        if constexpr (is_base_of_holder<O>)
        {
          ...
          holder_object.deserialize = [&](YAML::Node newroot) {
            ...
            for (YAML::iterator holder = vectorofholder.begin(); holder != vectorofholder.end(); ++holder)
            {
  ...
  public:
    holder() : holder_object{"int_value", "int_value_desc", "scalar", ObjectType::sequence, nullptr} { setup_sequence_holder(); }
    ...
    void push_back(const O &value)
    {
      if constexpr (is_base_of_holder<O>)
      {
        ...
        holder_object.parent->node[holder_object.name].push_back(value.holder_object.node);
        holder_object.parent->node[holder_object.name].SetTag(holder_object.type_name);
      }
      else
      {
...
public:                                                                                                                                              \
  property::Sequence<param_type> param_name{#param_name, param_desc,                                                                                 \
                                            (std::string("vector:") + std::string(property::deduce_prop_type_name<param_type>(#param_type))).data(), \
                                            &this->holder_object};
