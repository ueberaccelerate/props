...
namespace property
{
  ...
  public:
    holder() : holder{"default_name", "default_desc", "serializable", nullptr} {}
    holder(const char *name, const char *desc, const char *type_name, SerializeNode *parent)
  ...
  public:
    void setParent(SerializeNode *parent) { parent->childs.insert(SerializeNodePtr(&holder_object, StackDeleter<SerializeNode>{})); }
    ...
    void deserialize(PROPERTY_UNUSED const std::string &serdata) { deserialize(YAML::Load(serdata)); }
    ...
    void deserialize(PROPERTY_UNUSED YAML::Node root)
    {
      ...
      if (root.size() != holder_node.size())
        throw size_serialize_error();
      ...
      for (YAML::iterator ri = std::next(root.begin()), hi = std::next(holder_node.begin()); ri != root.end(); ++ri, ++hi)
      {
