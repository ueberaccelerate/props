...
private:
  void setup_map_holder() {
    if (holder_object.parent) {
      ...
      holder_object.parent->node[holder_object.name + "_doc"] =
          holder_object.desc;
      ...
      if constexpr (is_base_of_holder<O>) {
        ...
        holder_object.deserialize = [&](YAML::Node newroot) {
          ...
          for (YAML::const_iterator item = newvalue.begin();
               item != newvalue.end(); ++item) {
...
public:
  ...
  holder(const char *name_, const char *desc_,
    ...
    //            auto it = std::find_if(holder_object.type_name.begin(),
    //            holder_object.type_name.end(), [](const auto ch){return
    //            ch=='!';}); std::string
    //            type1(holder_object.type_name.begin(),it); std::string
    //            type2(std::next(it,2), holder_object.type_name.end());
...
inline std::string concatenate_with_string_inmiddle(const std::string &f,
                                                    const std::string &s,
                                                    const std::string &m) {
  return f + m + s;
}
...
#define serializable_m(param_name, param_type1, param_type2, param_name_text,  \
                       param_desc)                                             \
public:                                                                        \
  property::Map<param_type1, param_type2> param_name{                          \
      #param_name, param_desc,                                                 \
      (std::string("map:") +                                                   \
       property::concatenate_with_string_inmiddle(                             \
           property::deduce_prop_type_name<param_type1>(#param_type1).data(),  \
